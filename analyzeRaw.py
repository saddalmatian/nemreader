...
def lambda_handler(event, context):
    ...
    header = raw_data_container['header']
    readings = raw_data_container['readings']
    transactions = raw_data_container['transactions']
    f = open('/tmp/raw/Result.txt', 'w')
    result_nem = nem_objects.NEMFile(header, readings, transactions)
    for nmi in result_nem.readings:
        for suffix in result_nem.readings[nmi]:
            for reading in result_nem.readings[nmi][suffix]:
                f.write(str(reading)+"\n")
    f.close()
    f = open('/tmp/raw/Result.txt', 'r')
