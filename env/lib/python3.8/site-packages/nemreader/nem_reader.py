...
def read_nem_file(file_path: str, quantity:str, ignore_missing_header=False) -> NEMFile:
...
def parse_nem12_rows_continue(
    ...
    if not observed_900_record:
        if not observed_900_record:
            ID_VERSION += 1
            file_name=PREFIX_FILE+str(ID_VERSION)+".csv"
            print(file_name)
            if(ID_VERSION!=FILE_QUANTITY):
                with open(file_name) as nmi_file:
                    return parse_nem12_rows_continue(nem_list, header=header, file_name=nmi_file)
