from nemreader.nem_objects import HeaderRecord
...
def split_nem12_file(file_path:str, line_quantity: int):
    ...
    with open(file_path,'r') as data_file:
        for line in data_file:
            ...
            if(count>line_quantity and list(line.split(","))[0] != "200" or current_line==max_line):
...
def main():
    ...
    split_nem12_file('NEM12.csv',4)
    for file_id in range(1,FILE_QUANTITY):
        if(file_id == 1):
            m = read_nem_file(PREFIX_FILE+str(file_id)+".csv")            
        else:
            m += read_nem_file(PREFIX_FILE+str(file_id)+".csv",ignore_missing_header=True)    
